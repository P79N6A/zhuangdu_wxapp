...
<?php  if(count($result['data']['list'])>0) { ?>
	...
	<table class="table table-hover table-responsive">
		...
		<tbody>
			...
			<?php  if(is_array($result['data']['list'])) { foreach($result['data']['list'] as $yoxdiy) { ?>
			<tr>
				<td>
					<input type='checkbox' value="<?php  echo $yoxdiy['id'];?>" />
				</td>
				<td>
					<?php if(cv('yoxdiy.edit')) { ?>
						<a href='javascript:;' data-toggle='ajaxEdit' data-href="<?php  echo webUrl('yoxdiy/displayorder',array('id'=>$yoxdiy['id']))?>"><?php  echo $yoxdiy['displayorder'];?></a> 
					<?php  } else { ?>
						<?php  echo $yoxdiy['displayorder'];?> 
					<?php  } ?>
				</td>
				<td><?php  echo $yoxdiy['name'];?></td>
				<td><?php  echo $yoxdiy['description'];?></td>
				<!--<td>
					<span class='label <?php  if($yoxdiy['status']==1) { ?>label-primary<?php  } else { ?>label-default<?php  } ?>' 
						<?php if(cv('yoxhotsearch.page.edit')) { ?> 
							data-toggle="ajaxSwitch" 
							data-confirm = "确认<?php  if($yoxdiy['status']==1) { ?>关闭<?php  } else { ?>开启<?php  } ?>吗？"
							data-switch-value="<?php  echo $yoxdiy["yoxhotsearch_status"];?>" 
						...
						<?php  if($yoxdiy['status']==1) { ?>开启<?php  } else { ?>关闭<?php  } ?>
					</span>
				</td>-->
				<td>
					<?php if(cv('yoxdiy.edit')) { ?>
