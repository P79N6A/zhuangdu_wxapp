<?php defined('IN_IA') or exit('Access Denied');?><div class="region-goods-details row">
    ...
    <div class="col-sm-9 col-xs-12">
        ...
        <?php  if(p('yoxwechatbusiness')) { ?>
        <table>
        <tr><td width="80">微商等级</td><td width="80">价格</td><td>上等级返(小于0是返百分比,大于0是返X元)</td><td>上级返(小于0是返百分比,大于0是返X元)</td></tr>
        <?php  if(is_array($yoxwechatbusiness_level_list)) { foreach($yoxwechatbusiness_level_list as $level_info) { ?>
        <tr><td><?php  echo $level_info['level'];?>-<?php  echo $level_info['name'];?></td>
        <td><input type="text" name="marketprice_level_<?php  echo $level_info['level'];?>" id="marketprice_level_<?php  echo $level_info['level'];?>" class="form-control" value="<?php  echo $item['marketprice_level_'.$level_info['level']]?>"></td>
        <td><input type="text" name="commission_up_level_<?php  echo $level_info['level'];?>" id="commission_up_level_<?php  echo $level_info['level'];?>" class="form-control" value="<?php  echo $item['commission_up_level_'.$level_info['level']]?>"></td>
        <td><input type="text" name="commission_invite_level_<?php  echo $level_info['level'];?>" id="commission_invite_level_<?php  echo $level_info['level'];?>" class="form-control" value="<?php  echo $item['commission_invite_level_'.$level_info['level']]?>"></td></tr>
        <?php  } } ?>
        </table>
        <?php  } ?>
    </div>
