...
class Weisrc_dishModuleSite extends Core
{
    ...
    {
        file_put_contents(IA_ROOT . "/addons/weisrc_dish/notice.log", date('Y-m-d H:i:s') . PHP_EOL, FILE_APPEND);
    }
...
BY id DESC", array(':weid' => $weid, ':from_user' => $from_user, ':couponid' => $id));
        ...
        if ($coupon_usercount != 0) {
            if ($user_count >= $coupon_usercount) {
                $this->showMsg("每人最多只能兑换{$coupon_usercount}次!");
        ...
        foreach ($settings as $item => $value) {
            ...
            if ($value['is_print_all'] == 1) {
                ...
                foreach ($ordergoods as $v) {
					...
					$str = '';
					$str .= '' . str_pad($goodsname, 23, ' ', STR_PAD_RIGHT);
					$str .= '' . $v['total'] . str_pad($goods[$v['goodsid']]['unitname'], 5, ' ', STR_PAD_RIGHT);
                ...
                foreach ($order['goods'] as $v) {
                    ...
                    $content1 .= "-------------------------------\n";
                    if ($jinyun_type == 1) {
                        $content1 .= '<S2>' . $goodsname . "</S2>\n";
                        $content1 .= '数量:<S2>' . $v['total'] . $goods[$v['goodsid']]['unitname'] . "</S2>\n";
                    } else {
                        $content1 .= '@@2' . $goodsname . "\n";
                        $content1 .= '@@2数量:' . $v['total'] . $goods[$v['goodsid']]['unitname'] . "\n";
                    }
...
//        $message = (string) $xml->message;
    ...
    public function doMobileAddQrcodeFood()
    {
        ...
        $total = 1; //更新数量
        $optype = 'add';
        $jumpurl = $host . 'app/' . $this->createMobileUrl('waplist', array('storeid' => $storeid, 'mode' => $mode), true);
