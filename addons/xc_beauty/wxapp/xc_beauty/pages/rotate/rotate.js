...
Page({
    ...
    startRollTap: function() {
        var n = this;
        if (n.data.rotated <= 0) wx.showModal({
            title: "您已经没有抽奖机会了",
            content: "欢迎下次再来",
            showCancel: !1,
            confirmText: "确定"
        }); else if (canRoll) {
            var o = this.aniData, t = Math.random() * parseInt(n.data.total_time);
            console.log("随机数是" + t);
            var r = void 0, a = n.data.prize;
            console.log(a);
            for (var e = 0; e < a.length; e++) t > parseInt(a[e].min) && t < parseInt(a[e].max) && (r = e);
            console.log(r), console.log("奖品是：" + lottery[r].name), app.util.request({
                url: "entry/wxapp/service",
                showLoading: !1,
                data: {
                    op: "sign_log",
                    id: n.data.lottery[r].id
                },
                success: function(t) {
                    if ("" != t.data.data) if (canRoll = !1, o.rotate(3600 * num - 360 / lotteryArrLen * r).step(), 
                    n.setData({
                        aniData: o.export()
                    }), num++, 3 != lottery[r].type) {
                        n.awardShow(lottery[r].name, lottery[r].type);
                        var a = n.data.annList, e = {
                            title: lottery[r].name,
                            nick: n.data.nick
                        };
                        a.push(e), n.setData({
                            annList: a
                        ...
                        wx.showModal({
                            title: lottery[r].name,
                            content: lottery[r].name,
                            showCancel: !1,
                            confirmText: "确定"
                        }), n.setData({
                            rotated: parseInt(n.data.rotated) - 1
                        }), canRoll = !0;
                    }, 2100);
                }
            });
