...
Page({
    ...
    onLoad: function(s) {
        var i = this;
        common.config(i), common.theme(i), app.util.request({
            url: "entry/wxapp/service",
            data: {
                op: "pclass"
            },
            success: function(t) {
                ...
                if ("" != a.data) {
                    ...
                    if (null != s.cid && null != s.cid) {
                        for (var r = 0; r < a.data.length; r++) a.data[r].id == s.cid && (e = r);
                        i.setData({
                            sortCurr: e,
    ...
    onShareAppMessage: function() {
        ...
        var r = "";
        "" != t.data.config.share_service_img && null != t.data.config.share_service_img && (r = t.data.config.share_service_img);
        var s = "/xc_beauty/pages/base/base?&share=" + a, i = 1;
        return "" != app.share && null != app.share && "" != app.share.content && null != app.share.content && (i = app.share.content.status), 
        1 == i && (s = s + "&scene=" + app.userinfo.openid), {
            title: e,
            path: s,
            imageUrl: r,
            success: function(t) {
