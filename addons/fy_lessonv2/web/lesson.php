...
if ($operation == 'display') {
	...
	/* 订单会员 */
	$order_list = pdo_fetchall("SELECT b.openid FROM " .tablename($this->table_order). " a LEFT JOIN " .tablename($this->table_fans). " b ON a.uid=b.uid WHERE a.uniacid=:uniacid AND a.teacherid=:teacherid", array(':uniacid'=>$uniacid, ':teacherid'=>$lesson['teacherid']));
	/* VIP会员 */
	$vip_list = pdo_fetchall("SELECT b.openid FROM " .tablename($this->table_member_vip). " a LEFT JOIN " .tablename($this->table_fans). " b ON a.uid=b.uid WHERE a.uniacid=:uniacid AND a.validity>=:validity", array(':uniacid'=>$uniacid,':validity'=>time()));
	/* 合并会员 */
	$list = array_merge($order_list, $vip_list);
	/* 去除数组中重复记录 */
	$list = $this->two_array_unique($list);

	$tplmessage = pdo_fetch("SELECT newlesson FROM " .tablename($this->table_tplmessage). " WHERE uniacid=:uniacid", array(':uniacid'=>$uniacid));
	$number = 0;
	foreach($list as $student){
		...
		$sendmessage = array(
			...
			'data'        => array(
				 'first'  => array(
					 'value' => "您好，您关注的{$lesson['teacher']}讲师课程更新啦~",
					 'color' => "#222222",
				 ),
				 'keyword1' => array(
	...
	$list = pdo_fetchall("SELECT a.uid,a.playtime,a.addtime,b.bookname,c.title FROM " .tablename($this->table_playrecord). " a LEFT JOIN " .tablename($this->table_lesson_parent). " b ON a.lessonid=b.id LEFT JOIN " .tablename($this->table_lesson_son). " c ON a.sectionid=c.id WHERE {$condition} ORDER BY a.id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
	foreach($list as $key=>$value){
		$list[$key]['user'] = pdo_fetch("SELECT nickname FROM " .tablename('mc_members'). " WHERE uid=:uid", array(':uid'=>$value['uid']));
		$list[$key]['playtime'] = gmdate("H:i:s", $list[$key]['playtime']);
	}

	$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " .tablename($this->table_playrecord). " a LEFT JOIN " .tablename($this->table_lesson_parent). " b ON a.lessonid=b.id LEFT JOIN " .tablename($this->table_lesson_son). " c ON a.sectionid=c.id WHERE {$condition}", $params);
	$pager = pagination($total, $pindex, $psize);
