...
/* 查询是否收藏该课程 */
$collect = $collect = pdo_fetch("SELECT * FROM " .tablename($this->table_lesson_collect). " WHERE uniacid=:uniacid AND uid=:uid AND outid=:outid AND ctype=:ctype LIMIT 1", array(':uniacid'=>$uniacid,':uid'=>$uid,':outid'=>$teacherid,':ctype'=>2));
...
foreach($lesson_list as $key=>$value){
	$visit = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_lesson_history) . " WHERE lessonid=:lessonid", array(':lessonid'=>$value['id']));
	if($value['price']>0){
		$student_num += $value['virtual_buynum']+$value['buynum'];
	}else{
		$student_num += $value['virtual_buynum']+$value['buynum']+$visit;
	}
	$lesson_list[$key]['price'] = $value['price']>0 ? "yen".$value['price'] : '免费';
	if($value['price']>0){
