...
if ($op != 'allcategory') {
	...
	foreach ($list as $key => $value) {
		...
		$list[$key]['visit'] = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this -> table_lesson_history) . " WHERE lessonid=:lessonid", array(':lessonid'=>$value['id']));
		$list[$key]['price'] = $value['price']>0 ? "yen".$value['price'] : "免费";
		if($value['price']>0){
			$list[$key]['buyTotal'] = $value['buynum'] + $value['virtual_buynum'];
		}else{
			$list[$key]['buyTotal'] = $value['buynum'] + $value['virtual_buynum'] + $list[$key]['visit'];
		}
		if($value['score']>0){
