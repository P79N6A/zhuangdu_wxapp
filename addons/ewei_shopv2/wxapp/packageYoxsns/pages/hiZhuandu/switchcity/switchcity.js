...
Page({
  ...
  bindCity: function (e) {
    ...
    t.globalData.defaultCity = this.data.city
    t.globalData.defaultCounty = ''
  },
  bindCounty: function (e)  //设置当前区域
  {
    this.setData({ county: e.currentTarget.dataset.city })
    t.globalData.defaultCounty = this.data.county
    wx.reLaunch({
      url: '../index',
    })
  },
  ...
  onLoad: function (options) {
    const searchLetter = a.searchLetter;
     console.log(searchLetter)
    const cityList = a.cityList();
  ...
  selectCounty: function () {
    ...
    const that = this;
    wx.request({
      url: `https://apis.map.qq.com/ws/district/v1/getchildren?&id=${code}&key=${t.globalData.tencentMapKey}`,
      success: function (res) {
        that.setData({
