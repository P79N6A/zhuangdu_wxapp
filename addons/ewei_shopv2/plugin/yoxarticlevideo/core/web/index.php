...
class Index_EweiShopV2Page extends PluginWebPage 
{
    ...
    public function article_list(){
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_yoxarticlevideo_article') . " WHERE $where ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(), 'id DESC');
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_yoxarticlevideo_article') . " WHERE $where");
        ...
        $cate_list = pdo_fetchAll("SELECT * FROM " . tablename('ewei_shop_yoxarticlevideo_cate') . " WHERE uniacid = :uniacid", array( ':uniacid' => $uniacid));
	...
	public function edit() 
	{
	    ...
	    if($name!=''||$description||$thumbs){
	        ...
	        $name && $data['name']=$name;
	        $thumb&& $data['thumb']=serialize($thumb);
...
// 	        message('成功', $this->createWebUrl('yoxfriendscircle', array()), 'success');
	...
	public function cate_list(){
	    ...
	    $list = pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_yoxarticlevideo_cate') . " WHERE $where ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(), 'id DESC');
	    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_yoxarticlevideo_cate') . " WHERE $where");
	    $pager = pagination($total, $pindex, $psize);
	...
	public function cate_edit(){
	    ...
	    if($name!=''){
	        ...
	        !$id && pdo_insert('ewei_shop_yoxarticlevideo_cate', $data);
	        $id  && pdo_update("ewei_shop_yoxarticlevideo_cate", $data, array("id" => $id));
	        
	        show_json(1, "成功");
	        // 	        message('成功', $this->createWebUrl('yoxfriendscircle', array()), 'success');
	    }
	    
	    $info = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_yoxarticlevideo_cate') . " WHERE id = :id AND uniacid = :uniacid", array(':id' => $id, ':uniacid' => $uniacid));
	    $info['thumbs']=unserialize($info['thumbs']);
