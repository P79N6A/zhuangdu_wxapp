...
if (!(class_exists('SnsModel'))) 
{
	class SnsModel extends PluginModel 
	{
		 ...
		 * sns用户列表 、用户排行榜
		 */
		public function sns_member_list(){
		    global $_W;
		    global $_GPC;
		    $sns_member_list = pdo_fetchall('select * from ' . tablename('ewei_shop_sns_member') . ' where uniacid=:uniacid ORDER BY credit desc ', array(':uniacid' => $_W['uniacid']));
		    
		    return $sns_member_list;
		}
		/**
		 * 热搜
		 */
		public function set_hot_search($keyword){
		    global $_W;
		    //BOARD/TOPIC
		    $keyword_info = pdo_fetch('select * from ' . tablename('ewei_shop_sns_hotsearch') . ' where uniacid=:uniacid and `keyword`=:keyword  limit 1', array(':uniacid' => $_W['uniacid'],':keyword'=>$keyword));
		    if(empty($keyword_info)){
		        pdo_insert("ewei_shop_sns_hotsearch",array('uniacid'=>$_W['uniacid'],'type'=>'BOARD','keyword'=>$keyword,'is_hot'=>'0','status'=>1,'displayorder'=>0));
		    }
		    if($keyword_info){
		        $displayorder=$keyword_info['displayorder']+1;
		        $res = pdo_update('ewei_shop_sns_hotsearch', array('displayorder'=>$displayorder), array('uniacid' => $_W['uniacid'],'id'=>$keyword_info['id']));
		    }
		}
		public function hot_search_list(){
		    $keyword_list = pdo_fetchall('select * from ' . tablename('ewei_shop_sns_hotsearch') . ' where uniacid=:uniacid and   limit 0,20 DESC', array(':uniacid' => $_W['uniacid']));
		    $result['status']=1;
		    $result['message']="ok";
		    $result['data']['list']=$keyword_list;
		    return $keyword_list;
		}
	}
}
