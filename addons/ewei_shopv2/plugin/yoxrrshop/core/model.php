...
class yoxrrshopModel extends PluginModel
{
    ...
    public function page_template_list(){
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_yoxrrshop_page_template') . " page WHERE $where ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(), 'id DESC');
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_yoxrrshop_page_template') . " WHERE $where");
    ...
    public function action_template_delete(){
        ...
        $id = intval($_GPC["id"]);
        $uniacid=$_W['uniacid']?$_W['uniacid']:1;
        pdo_delete('ewei_shop_yoxrrshop_page_template', array('uniacid'=>$uniacid,'id' => $id));
    }
    //===========================以上是页面==========================================//
