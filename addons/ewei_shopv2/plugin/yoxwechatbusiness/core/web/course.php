...
class Course_EweiShopV2Page extends PluginWebPage 
{
    //列表
...
// 	        message('成功', $this->createWebUrl('yoxfriendscircle', array()), 'success');
	    ...
	    //课程分类
	    $p_cate_list=pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_yoxwechatbusiness_course_cate') . " course_cate WHERE course_cate.uniacid=:uniacid AND course_cate.pid=0  LIMIT 0,999",array(':uniacid'=>$uniacid));
	    
	    foreach($p_cate_list as $p_info){
	        $pid_arr[]=$p_info['id'];
	    }
	    $pid_str=implode(',', $pid_arr);
	    $pid_arr && $c_cate_list=pdo_fetchall("SELECT course_cate.* FROM " . tablename('ewei_shop_yoxwechatbusiness_course_cate') . " course_cate WHERE course_cate.uniacid=:uniacid AND course_cate.pid in($pid_str)  LIMIT 0,999" ,array(':uniacid'=>$uniacid));
	    
	    foreach($p_cate_list as $p_info){
	        foreach($c_cate_list as $c_info){
	...
	public function course_chapter_edit(){
	    ...
	    if($name&&$course_id){
	    if(empty($name)){show_json(0, "章节名称为空");}
	...
	public function course_message_edit()
	{
	    ...
	    foreach($image as &$img){
	        if(!strpos('https', $img)){$img=$_W['attachurl'].$img;}
	    }
	    foreach($voice as &$voi){
	        if(!strpos('https', $voi)){$voi=$_W['attachurl'].$voi;}
	    }
	    foreach($video as &$vid){
	        if(!strpos('https', $vid)){$vid=$_W['attachurl'].$vid;}
	    }
	    foreach($document as &$doc){
	        if(!strpos('https', $doc)){$doc=$_W['attachurl'].$doc;}
	    }
	    ...
	    $chapter_id=$chapter_id?$chapter_id:$info['chapter_id'];
	    $course_id=$course_id?$course_id:$info['course_id'];
	    
	    $info['uid'] && $user_info = pdo_fetch("select uid,nickname,avatar from " . tablename("mc_members") . " where uid=:uid and uniacid=:uniacid limit 1", array( ":uid" => $info['uid'], ":uniacid" => $_W["uniacid"] ));
	    //课程
	    $course_list=pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_yoxwechatbusiness_course') . " course WHERE course.uniacid=:uniacid LIMIT 0,9999",array(':uniacid'=>$uniacid));
	    
	    ($chapter_where=" course_chapter.uniacid=:uniacid ") && ($chapter_replace=array(':uniacid'=>$uniacid,));
	    //$chapter_id && ($chapter_where.=" AND course_chapter.id=:chapter_id") && ($chapter_replace+=array(':chapter_id'=>$chapter_id,));
	    $course_id && ($chapter_where.=" AND course_chapter.course_id=:course_id") && ($chapter_replace+=array(':course_id'=>$course_id,));
	    $course_chapter_list=pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_yoxwechatbusiness_course_chapter') . " course_chapter WHERE  $chapter_where LIMIT 0,9999",$chapter_replace);
	    ...
	    $result['data']['course_list']=$course_list;
	    $result['data']['course_chapter_list']=$course_chapter_list;
	...
	public function course_message_delete()
	{
	    ...
	    $items = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_yoxwechatbusiness_course_message") . " WHERE id in( " . $id . " ) AND uniacid=" .$uniacid);
	    foreach($items as $item){
	        pdo_delete('ewei_shop_yoxwechatbusiness_course_message', array('id' => $item['id'],'uniacid'=>$uniacid));
	    }
	    show_json(1,'删除成功');
	}
	/**
...
// 	        $data['update_time']=time();
	    ...
	    $course_cate_list=pdo_fetchall("SELECT course.* FROM " . tablename('ewei_shop_yoxwechatbusiness_course_cate') . " course WHERE course.uniacid=:uniacid AND course.pid=0  LIMIT 0,100" ,array(':uniacid'=>$uniacid));

	    $result['status']=1;
	    $result['message']='success';
	    $result['data']=$info;
	    $result['data']['course_cate_list']=$course_cate_list;
	    //pdo_debug();
	    ($_W['isajax']||$_GPC['isajax']) && die(json_encode($result));
