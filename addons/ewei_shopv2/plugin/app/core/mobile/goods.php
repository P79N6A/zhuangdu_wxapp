...
class Goods_EweiShopV2Page extends AppMobilePage
{
	...
	public function get_detail()
	{
		...
		$member = m("member")->getMember($openid);//print_r($_W);print_r($member);echo 'uid:'.mc_openid2uid($openid);die();
		...
		if(false && $yoxwechatbusiness){
		    ...
		    $yoxwechatbusiness_price=$yoxwechatbusiness->goods_price($goods['id'],0,$member['uid']);
		    $yoxwechatbusiness_price && ($yoxwechatbusiness_price<$goods["minprice"]) &&($goods["minprice"]=$yoxwechatbusiness_price);
		    $goods=array_merge((array)$goods,(array)$wechatbusiness_goods_field);
		    //echo $yoxwechatbusiness_price;print_r($member);die('Yoer');
		}
	...
	public function get_picker()
	{
		...
		if($yoxwechatbusiness){
		    $level_price=$yoxwechatbusiness->goods_price($goods['id'],0,$member['uid']);
		    $level_price>0 && ($goods["minprice"]=$level_price);
		}
		$goods["minprice"] = number_format($minprice, 2);
		$goods["maxprice"] = number_format($maxprice, 2);
