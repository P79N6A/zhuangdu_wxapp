...
class Shop_EweiShopV2Page extends AppMobilePage
{
	...
	public function get_category()
	{
		...
		$parent_ids && ($goods_list =pdo_fetchall("SELECT goods.* FROM " . tablename('ewei_shop_goods') . " goods WHERE goods.uniacid = {$_W['uniacid']} AND goods.pcate in($parent_ids) ORDER BY goods.id DESC LIMIT 0,9999  ", array()));
		...
		foreach ($category['parent'] as $p) {
			...
			foreach($goods_list as $goods_info){
			    if($p['id']==$goods_info['pcate']){
			        $parent['goods_list'][]=$goods_info;
			    }
			}
			$allcategory[] = $parent;
		}

		app_json(array('set' => $category_set, 'recommands' => $recommands, 'category' => $allcategory,'goods_list'=>$goods_list));
	}
