...
class Create_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
	{
		...
		if( !$packageid )
		{
			...
			if( empty($id) )
			{
				...
				foreach( $goods as $k => $v )
				{
					...
					if(p('yoxwechatbusiness')){
					    ...
					    $level_price=$yoxwechatbusiness->goods_price($v['goodsid'],$v['optionid'],$_W['uid']);
					    $level_price>0 && ($goods[$k]["marketprice"]=$level_price);
					}
			...
			$seckill_price = 0;//秒杀优惠了多少钱//
			$seckill_payprice = 0;//秒杀支付价
			$yoxwechatbusiness_price=0;//微商会员等级优惠了多少钱
			$yoxwechatbusiness_payprice=0;//微商等级支付价格
			$ismerch = 0;
			...
			foreach( $goods as &$g )
			{
				...
				{
					$gprice = $g["marketprice"] * $g["total"];//单商品*数量 原价格
					$g["ggprice"] = $g["seckillinfo"]["price"] * $g["total"];//单商品*数量 秒杀价格
					$seckill_payprice += $g["seckillinfo"]["price"] * $g["total"];//秒杀支付价格
					$seckill_price += $g["marketprice"] * $g["total"] - $seckill_payprice;//秒杀优惠了多少钱
				}
				elseif(p('yoxwechatbusiness')){//微商等级价格//
					$yoxwechatbusiness=p('yoxwechatbusiness');
					$level_price=$yoxwechatbusiness->goods_price($g['goodsid'],$g['optionid'],$_W['uid']);
					$level_price>0 && ($gprice=$g["marketprice"] * $g["total"]) && ($g["ggprice"] = $level_price* $g["total"]);
					$level_price>0 && ($yoxwechatbusiness_pay_price+= $level_price * $g["total"]);//微商等级支付价格
					$level_price>0 && ($yoxwechatbusiness_price+=$gprice-$yoxwechatbusiness_pay_price);//等级价格优惠了多少钱
				}
				else
				{
				    $gprice = $g["marketprice"] * $g["total"];//单商品*数量 原价格
		...
		else
		{
			...
			//是否可以购买套餐//
			if(p("yoxwechatbusiness")){
			    $setting_value= p("yoxwechatbusiness")->setting_value($uniacid, 'UPGRADE');
			    if($setting_value['setting']['type']=='UPGRADE_BY_PACKAGE_INVITED'){
			        $invited_count=p("yoxwechatbusiness")->invited_count($_W['member']['uid']);
			        $level_info = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_yoxwechatbusiness_level') . " WHERE uniacid=:uniacid AND package_id = :package_id ", array(':uniacid'=>$uniacid,':package_id' =>$packageid));
			        if($invited_count<$setting_value['setting']["level_{$level_info['level']}_number"]){
			            $cha=$setting_value['setting']["level_{$level_info['level']}_number"]-$invited_count;
			            app_error(0,"您当前邀请了{$invited_count}人,还差{$cha}人,不能购买此套餐升级！");
			        }
			    }
			}
			$goods = array( );
			...
			foreach( $g as $key => $value )
			{
				...
				if( 0 < $value["optionid"] )
				{
					$option = pdo_fetch("select title,packageprice,marketprice,num from " . tablename("ewei_shop_package_goods_option") . "\r\n                            where optionid = " . $value["optionid"] . " and goodsid=" . $value["goodsid"] . " and uniacid = " . $uniacid . " and pid = " . $packageid . " ");
					$goods[$key]["packageprice"] = $option["packageprice"];
				}
				else
				{
					$packagegoods = pdo_fetch("select title,packageprice,marketprice,num from " . tablename("ewei_shop_package_goods") . "\r\n                            where goodsid=" . $value["goodsid"] . " and uniacid = " . $uniacid . " and pid = " . $packageid . " ");
				...
				$goods[$key]["total"] = !empty($option["num"]) ? $option["num"] : $packagegoods['num'];//
				empty($goods[$key]["total"]) && ($goods[$key]["total"]=1);//
				if( $option )
				{
					$goods[$key]["packageprice"] = $option["packageprice"];
				}
		...
		foreach( $goods_list as $k => $v )
		{
			...
			{
			    $yoxwechatbusiness=p('yoxwechatbusiness');//
			    if($yoxwechatbusiness){
			        $yoxwechatbusiness_level_list = $yoxwechatbusiness->level_list($_W['uniacid']);
// 			        $wechatbusiness_goods_field=$yoxwechatbusiness->goods_field($goods['id']);
// 			        $goods['yoxwechatbusiness_level_list']=$yoxwechatbusiness_level_list;
			        $level_price=$yoxwechatbusiness->goods_price($g['goodsid'],0,$_W['uid']);
// 			        $level_price && ($g['level_price']=$level_price);
// 			        $goods=array_merge((array)$goods,(array)$wechatbusiness_goods_field);
			    }
				$allgoods[$k]["goods"][] = array( "id" => $g["goodsid"], "goodsid" => $g["goodsid"], "title" => $g["title"], "thumb" => tomedia($g["thumb"]), "optionid" => (int) $g["optionid"], "optiontitle" => $g["optiontitle"],
				    "hasdiscount" => empty($g["isnodiscount"]) && !empty($g["dflag"]), "total" => $g["total"], "price" => ($g["unitprice"] < $g["marketprice"] ? (double) $g["marketprice"] : (double) $g["unitprice"]),
				    "marketprice" => (double) $g["marketprice"], 'level_price'=>$level_price,"merchid" => $g["merchid"], "cates" => $g["cates"], "unit" => $g["unit"], "totalmaxbuy" => $g["totalmaxbuy"], "minbuy" => $g["minbuy"], "promotionprice" => (($g["unitprice"] < $g["marketprice"] ? (double) $g["marketprice"] : (double) $g["unitprice"])) - $g["isdiscountprice"] );
			}
		}
		...
		$result['yoxwechatbusiness_price']=$yoxwechatbusiness_price;
		$result['yoxwechatbusiness_payprice']=$yoxwechatbusiness_pay_price;
		if( $hasinvoice )
		{
	...
	public function caculate()
	{
		...
		$seckill_price = 0;
		$seckill_payprice = 0;
		$seckill_dispatchprice = 0;//秒杀派遣价？？
		$yoxwechatbusiness_price=0;//微商会员等级优惠了多少钱//
		$yoxwechatbusiness_payprice=0;//微商会员等级支付价格
		...
		else
		{
			if( is_array($goodsarr) )
			{
				...
				foreach( $goodsarr as &$g )
				{
					...
					if( $data["seckillinfo"] && $data["seckillinfo"]["status"] == 0 )
					{
						$data["ggprice"] = $data["seckillinfo"]["price"] * $g["total"];//秒杀支付单价格*数量
						$seckill_payprice += $data["ggprice"];//秒杀总支付价格
						$seckill_price += $data["marketprice"] * $g["total"];//原价格
					}
					elseif(p('yoxwechatbusiness')){//微商等级价格//
					    $yoxwechatbusiness=p('yoxwechatbusiness');
					    $level_price=$yoxwechatbusiness->goods_price($g['goodsid'],$g['optionid'],$_W['uid']);
					    $level_price>0 && ($gprice=$g["marketprice"] * $g["total"]) && ($data["ggprice"] = $level_price* $g["total"]);
					    $level_price>0 && ($yoxwechatbusiness_pay_price+= $level_price * $g["total"]);//微商等级支付价格
					    $level_price>0 && ($yoxwechatbusiness_price+=$gprice-$yoxwechatbusiness_pay_price);//等级价格优惠了多少钱
					}
					else
		...
		$return_array["yoxwechatbusiness_price"] = $yoxwechatbusiness_price;//
	...
	public function submit()
	{
		...
		$seckill_payprice = 0;
		$seckill_dispatchprice = 0;
		$yoxwechatbusiness_price=0;//
		$yoxwechatbusiness_payprice=0;//
		$buyagain_sale = true;
		$buyagainprice = 0;
		$sale_plugin = com("sale");
		$saleset = false;
		...
		foreach( $goods as $g )
		{
			...
			if(p('yoxwechatbusiness')){
			    $yoxwechatbusiness=p('yoxwechatbusiness');
			    $level_price=$yoxwechatbusiness->goods_price($data['goodsid'],$data['optionid'],$_W['uid']);
			    $level_price>0 && ($data["ggprice"]=$gprice=$level_price * $goodstotal);
			    $level_price>0 && ($yoxwechatbusiness_payprice+= $gprice);//微商等级支付价格
			    $level_price>0 && ($yoxwechatbusiness_price+= $data["marketprice"] * $goodstotal - $gprice);//优惠了多少钱
			    $level_price>0 && ($goodsprice+= $data["marketprice"] * $goodstotal);//商品原总价
			}
			//秒杀//
			elseif( $data["seckillinfo"] && $data["seckillinfo"]["status"] == 0 )
			{
				$data["ggprice"] = $gprice = $data["seckillinfo"]["price"] * $goodstotal;//秒杀总个总价
				$seckill_payprice += $gprice;//秒杀支付总价
				$seckill_price += $data["marketprice"] * $goodstotal - $gprice;//秒杀总共优惠了多少钱
				$goodsprice += $data["marketprice"] * $goodstotal;//商品原总价
		...
		$order["up_level_uid"]=pdo_fetchcolumn('select up_level_invite_uid from '.tablename('ewei_shop_yoxwechatbusiness_user').' where uid='.$member['uid']);
		...
		if( $multiple_order == 0 )
		{
			foreach( $allgoods as $goods )
			{
				...
				$order_goods["uid"] = $_W['member']['uid'];//Yoper add//
