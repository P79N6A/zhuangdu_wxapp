...
class Board_EweiShopV2Page extends Base_EweiShopV2Page
{
	...
	public function best()
	{
		...
		$data['data']['isManager']=$isManager;
		$data['data']['shopshare']=$_W['shopshare'];
		($_W['isajax'] || $_GPC['isajax']) && show_json(1, array('data' => $data));
		$_GPC['isdebug']&& print_r($data) && die();
	...
	public function getlist()
	{
		...
		if($keyword){
		    ...
		    $this->model->set_hot_search($_GPC['keywords'],'POST');
		}
		//有微商模块，就只能看上等级发布的
		if(p('yoxwechatbusiness')){
		    $yoxwechatbusiness = p('yoxwechatbusiness');
		    $up_level_user_info = $yoxwechatbusiness->up_level_user($openid);
		    $up_level_user_info['openid'] && ($condition.= " AND openid={$up_level_user_info['openid']} ");
		}
		
		$sql = 'select id,title,createtime,content,images , nickname,avatar,isbest,isboardbest,checked,openid from ' . tablename('ewei_shop_sns_post') . ('  where 1 ' . $condition . ' ORDER BY createtime desc,id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
		...
		foreach ($list as &$row) {
			...
			$row['uid']=mc_openid2uid($row['openid']);
	...
	public function lists()
	{
		...
		($_W['isajax'] || $_GPC['isajax']) && show_json(1, array('data' => $category));
		$_GPC['isdebug']&& print_r($category) && die();
	...
	public function get_boardlist()
	{
		...
		if (!empty($_GPC['keywords'])) {
		    $this->model->set_hot_search($_GPC['keywords'],'BOARD');
