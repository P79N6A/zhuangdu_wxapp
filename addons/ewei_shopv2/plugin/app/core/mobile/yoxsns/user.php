...
class User_EweiShopV2Page extends Base_EweiShopV2Page
{
	public function main()
	{
		...
		if (empty($id)) {
			if (!$this->islogin) {
				...
				($_W['isajax'] || $_GPC['isajax']) && show_json(0, array('message' => 'no login'));
				$_GPC['isdebug']&& print_r(array('message' => 'no login')) && die();
			}
		...
		if (empty($member)) {
		    ($_W['isajax'] || $_GPC['isajax']) && show_json(0, array('message' => 'member empty'));
		    $_GPC['isdebug']&& print_r(array('message' => 'member empty')) && die();
		}
	...
	public function boards()
	{
		...
		($_W['isajax'] || $_GPC['isajax']) && show_json(1, array('data' => $member));
		$_GPC['isdebug']&& print_r($member) && die();
