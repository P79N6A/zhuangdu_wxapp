...
class Goods_EweiShopV2Page extends AppMobilePage
{
	...
	public function fight_groups()
	{
		...
		$goods = set_medias($goods, "thumb");
		$groups_set=pdo_get("ewei_shop_groups_set",array('uniacid'=>$_W['uniacid']));
		if ($groups_set['just_up_level']==1) {
			$user=pdo_get("ewei_shop_yoxwechatbusiness_user",array('uniacid'=>$_W['uniacid'],'openid'=>$openid));
			$up_level_user=pdo_get("ewei_shop_yoxwechatbusiness_user",array('uniacid'=>$_W['uniacid'],'uid'=>$user['invite_uid']));
			$teams = pdo_fetchall("select o.paytime,o.id,o.goodid,o.teamid,m.nickname,m.realname,m.mobile,m.avatar,g.endtime,g.groupnum,g.thumb_url from " . tablename("ewei_shop_groups_order") . " as o\r\n\t\t\t\tleft join " . tablename("ewei_shop_member") . " as m on m.openid=o.openid and m.uniacid =  o.uniacid\r\n\t\t\t\tleft join " . tablename("ewei_shop_groups_goods") . " as g on g.id = o.goodid\r\n\t\t\t\twhere o.goodid = :goodid and o.uniacid = :uniacid and o.openid != :openid  and o.openid = :up_level_openid and o.deleted = 0 and o.heads = 1 and o.paytime > 0 and o.success = 0 and o.ladder_id = :ladder_id limit 10 ", array( ":goodid" => $goods["id"], ":uniacid" => $uniacid, ":openid" => $openid, "ladder_id" => $ladder_id ,":up_level_openid"=>$up_level_user['openid']));
		}else{
	...
	public function check_tuan()
	{
		...
		if ($groups_set['just_up_level']==1) {
			...
			if( $ladder_id )
			{
				$data["ladder_num"] = pdo_count("ewei_shop_groups_order", "uniacid = " . $uniacid . " and goodid = " . $goods_id . " and heads = 1 and success = 0 and openid != '" . $openid . "' and openid='".$up_level_user['openid']."' and ladder_id = " . $ladder_id);
			}
			$data["order_num"] = pdo_count("ewei_shop_groups_order", "uniacid = " . $uniacid . " and goodid = " . $goods_id . " and heads = 1 and success = 0  and openid='".$up_level_user['openid']."' and openid != '" . $openid . "'");
		}else{
			if( $ladder_id )
			{
