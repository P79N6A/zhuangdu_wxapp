	...
	public function __construct()
	{
		...
		if( $_GPC["openid"] != "sns_wa_" )
		{
			...
			if($yoxwechatbusiness){
			    $mc_member_info=$yoxwechatbusiness->shop_member_to_sys_member($member["openid"]);//把小程序用户加到系统里，获得uid
			    $yoxwechatbusiness->set_user();
			    $_W["uid"]= $_W['member']["uid"]=$_W['fans']["uid"]= $mc_member_info["uid"];//print_r($mc_member_info);
			}
			if( $this->iswxapp )
			{
