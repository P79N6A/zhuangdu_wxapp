		...
		if( !$packageid )
		{
			...
			if( empty($id) )
			{
				...
				foreach( $goods as $k => $v )
				{
					...
					if(p('yoxwechatbusiness')){
					    ...
					    $level_price=$yoxwechatbusiness->goods_price($v['goodsid'],$v['optionid'],$_W['fans']['uid']);
			...
			else
			{
				...
				else
				{
					...
					if(p('yoxwechatbusiness')){
					    $yoxwechatbusiness=p('yoxwechatbusiness');
					    $level_price=$yoxwechatbusiness->goods_price($data["goodsid"],0,$_W['fans']['uid']);
					    $level_price>0 && ($data["marketprice"]=$level_price);
					}
					if( !empty($liveid) )
					{
					...
					if( !empty($optionid) )
					{
						...
						if( !empty($option) )
						{
							...
							if(p('yoxwechatbusiness')){
							$yoxwechatbusiness=p('yoxwechatbusiness');
							$level_price=$yoxwechatbusiness->goods_price($data['goodsid'],$option['id'],$_W['fans']['uid']);
							$level_price>0 && ($data["marketprice"]=$level_price);
							}
			...
			foreach( $goods as &$g )
			{
				...
				elseif(p('yoxwechatbusiness')){//微商等级价格//
					$yoxwechatbusiness=p('yoxwechatbusiness');
					$level_price=$yoxwechatbusiness->goods_price($g['goodsid'],$g['optionid'],$_W['fans']['uid']);
			...
			if(p("yoxwechatbusiness")){
			    ...
			    if($setting_value['setting']['type']=='UPGRADE_BY_PACKAGE_INVITED'){
			        ...
			        if($invited_count<$setting_value['setting']["level_{$level_info['level']}_number"]){
			            ...
			            $this->message("您当前邀请了{$invited_count}人,还差{$cha}人,不能购买此套餐升级！", "", "error");
			        }
			    }
			}
			$goods = array( );
			...
			if( !empty($optionid) )
			{
				...
				if( !empty($option) )
				{
					...
					if( $data["type"] != 4 )
					{
						...
						if(p('yoxwechatbusiness')){
						    $yoxwechatbusiness=p('yoxwechatbusiness');
						    $level_price=$yoxwechatbusiness->goods_price($data['goodsid'],$optionid,$_W['fans']['uid']);
						    $level_price>0 && ($data["marketprice"]=$level_price);
						}
						$packageoption = array( );
						if( $packageid )
