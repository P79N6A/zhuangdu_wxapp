...
class Picker_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
		...
		if(p('yoxwechatbusiness')){
		    ...
		    $level_price=$yoxwechatbusiness->goods_price($goods['id'],0,$_W['fans']['uid']);
		    $level_price>0 && ($goods["minprice"] = $level_price);
		    $level_price>0 && ($goods["maxprice"] = $level_price);
		}
		...
		//微商等级价格//
		if(p('yoxwechatbusiness')&& !empty($options) ){
		    $yoxwechatbusiness=p('yoxwechatbusiness');
		    foreach( $options as &$option )
		    {
		        $level_price=$yoxwechatbusiness->goods_price($option['goodsid'],$option['id'],$_W['fans']['uid']);
		        $level_price>0 && $option["marketprice"]=$level_price;
		        $level_price>0 && ($prices[]=$level_price);
		    }
		    unset($option);
		    $level_price>0 && ($goods["minprice"] = min($prices));
		    $level_price>0 && ($goods["maxprice"] = max($prices));
		}
