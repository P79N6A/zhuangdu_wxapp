...
class Index_EweiShopV2Page extends WebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				foreach ($goodsid as $key => $value) {
					...
					if (!$thisgoods && 7 <= $packagenum) {
						show_json(0, '同一件商品最多参与7个套餐活动!');
					}
				}
				...
				foreach ($goodsid as $key => $value) {
					...
					if (!empty($good_data['option'])) {
						...
						foreach ($packageGoodsOption as $k => $val) {
							...
							$optionData = array('uniacid' => $uniacid, 'goodsid' => $value, 'pid' => $id, 'title' => $op['title'], 'optionid' => $val, 'marketprice' => $op['marketprice'], 'packageprice' => $optionArray[0], 'commission1' => $optionArray[1], 'commission2' => $optionArray[2], 'commission3' => $optionArray[3],'num'=>$optionArray[4]);
		...
		if ($item) {
			...
			$package_goods = array();
			$package_goods = pdo_fetchall("select id,pid,title,thumb,packageprice,num,hasoption,goodsid,`option`,commission1,commission2,commission3\r\n                            from " . tablename('ewei_shop_package_goods') . ' where pid = ' . $id . ' and uniacid = ' . $uniacid . ' ');

			foreach ($package_goods as $key => $value) {
				if ($value['hasoption']) {
					$package_goods[$key]['optiontitle'] = pdo_fetchall("select id,goodsid,optionid,pid,packageprice,num,title,marketprice,commission1,commission2,commission3\r\n                            from " . tablename('ewei_shop_package_goods_option') . ' where pid = ' . $id . ' and goodsid = ' . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
				}
			}
		}
	...
	public function hasoption()
	{
		...
		if (!empty($pid)) {
			$packgoods = pdo_fetch('select id,title,packageprice,num,commission1,commission2,commission3,`option`,goodsid from ' . tablename('ewei_shop_package_goods') . "\r\n                        where pid = " . $pid . ' and uniacid = :uniacid and goodsid = :goodsid ', $params);
		}
		else {
			$packgoods = array('title' => $goods['title'], 'marketprice' => $goods['marketprice'], 'num'=>1,'packageprice' => 0, 'commission1' => 0, 'commission2' => 0, 'commission3' => 0);
		}
		...
		if ($goods['hasoption']) {
			...
			$package_option = pdo_fetchall('SELECT id,uniacid,goodsid,optionid,pid,title,marketprice,packageprice,num,commission1,commission2,commission3 FROM ' . tablename('ewei_shop_package_goods_option') . "\r\n            WHERE uniacid = :uniacid and goodsid = :goodsid  and pid = " . $pid . ' ', $params);
