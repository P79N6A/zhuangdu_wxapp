...
//微商Yoper add
$yoxwechatbusiness_plugin=p("yoxwechatbusiness");//
if($yoxwechatbusiness_plugin)
{
    $yoxwechatbusiness=p("yoxwechatbusiness");
    $yoxwechatbusiness_level_list = $yoxwechatbusiness->level_list($_W['uniacid']);
    $wechatbusiness_goods_field=$yoxwechatbusiness->goods_field($item['id'],'');
    $item=array_merge((array)$item,(array)$wechatbusiness_goods_field);
}
$status = $item["status"];
...
if( $_W["ispost"] )
{
	...
	if( empty($id) )
	{
		...
		if($yoxwechatbusiness){//
		    $yoxwechatbusiness->set_goods_field($id,'','');//Yoper add
		}
