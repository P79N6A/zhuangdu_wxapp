...
class Upgrade_EweiShopV2Page extends SystemPage
{
	...
	public function check()
	{
		...
		set_time_limit(0);
		$auth = get_auth();
		if(!$auth['code'])show_json(0, '&#x5F00;&#x5427;&#x6E90;&#x7801;&#x793E;&#x533A;&#x63D0;&#x9192;&#x60A8;&#xFF0C;&#x8BF7;&#x70B9;&#x6B64;&#x67E5;&#x770B;&#x65B0;&#x7248;&#xFF01;<a href="http://dwz.cn/pOvPbkyd">&#x7ACB;&#x5373;&#x67E5;&#x8BE2;</a>');
		$version = ((defined('EWEI_SHOPV2_VERSION') ? EWEI_SHOPV2_VERSION : '3.0.0'));
		$release = ((defined('EWEI_SHOPV2_RELEASE') ? EWEI_SHOPV2_RELEASE : '20180910001'));
		$resp = ihttp_request('http://sq.0516city.com/mod.php?a=mod&b=check', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				$resp = ihttp_request('http://sq.0516city.com/mod.php?a=mod&b=download', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
	...
	protected function updateComplete($version, $release)
	{
		...
		$sql ="UPDATE  `ims_modules` SET  `version` =  '$version',`author` =  '悟空源码社区',`url` =  'http://www.5kym,com/' WHERE  `name` ='ewei_shopv2'";
		pdo_query($sql);
	...
	{
		file_put_contents(IA_ROOT . '/addons/ewei_shopv2/version.php', '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'EWEI_SHOPV2_VERSION\')) {define(\'EWEI_SHOPV2_VERSION\', \'3.9.31\');}if(!defined(\'EWEI_SHOPV2_RELEASE\')) {define(\'EWEI_SHOPV2_RELEASE\', \'201809100001\');}');
