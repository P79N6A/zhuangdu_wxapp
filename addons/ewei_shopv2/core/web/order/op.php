...
class Op_EweiShopV2Page extends WebPage 
{
	...
	public function send() 
	{
		global $_W;
		global $_GPC;
		$small_securitycode=$_GPC['small_securitycode'];//Yoper add
		$small_codes=$_GPC['small_codes'];//数组形式 Yoper add
		$small_codes_str=$_GPC['small_codes_str'];//逗号分割形式 Yoper add
		$opdata = $this->opData();
		extract($opdata);
		if( empty($item["addressid"]) ) 
		{
		...
		if( $_W["ispost"] ) 
		{
			if( $item["city_express_state"] == 0 ) 
			{
				...
				($small_codes||$small_codes_str) && (pdo_delete("ewei_shop_order_goods_securitycode",array('uniacid'=>$_W["uniacid"],'order_id'=>$item["id"])));//Yoper add
				//数组//
				foreach ($small_codes as $key=>$small_codes2){
    				foreach($small_codes2 as $key2=>$small_code){
    				    pdo_insert("ewei_shop_order_goods_securitycode",array('uniacid'=>$_W["uniacid"],'order_id'=>$item["id"],'order_goods_id'=>$key,'small_code'=>$small_code));
    				}	
				}
				//分割.textarea中换行用的是 "\n"或"\r\n",(linux下输入为'\r\n',windows下为'\n') //
				$small_codes_str = str_replace("\r\n","\n",$small_codes_str);//
				$small_codes_arr=explode("\n", $small_codes_str);
				foreach($small_codes_arr as $small_code){
				    $small_code=trim($small_code);
				    $small_code && (pdo_insert("ewei_shop_order_goods_securitycode",array('uniacid'=>$_W["uniacid"],'order_id'=>$item["id"],'order_goods_id'=>0,'small_code'=>$small_code)));
				}
		...
		if( 0 < $item["sendtype"] ) 
		{
			...
			for( $i = 1; $i <= $item["sendtype"]; $i++ ) 
			{
				...
				$shipped[$i]["goods"] = pdo_fetchall("select g.id,g.title,g.thumb,og.sendtype,og.id as order_goods_id,g.ispresell from " . tablename("ewei_shop_order_goods") . " og " . " left join " . tablename("ewei_shop_goods") . " g on g.id=og.goodsid " . " where og.uniacid=:uniacid and og.sendtype = " . $i . " and og.orderid=:orderid ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $item["id"] ));
			}
		}
		$order_goods = pdo_fetchall("select og.id,og.total,g.title,g.thumb,og.sendtype,g.ispresell from " . tablename("ewei_shop_order_goods") . " og " . " left join " . tablename("ewei_shop_goods") . " g on g.id=og.goodsid " . " where og.uniacid=:uniacid and og.orderid=:orderid ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $item["id"] ));
		//安全码//
		$order_goods_securitycode_list=pdo_fetchall("select * from " . tablename("ewei_shop_order_goods_securitycode") . " order_goods_securitycode WHERE uniacid=:uniacid AND order_id=:order_id ",array(":uniacid"=>$_W["uniacid"],":order_id"=>$item["id"]));
		//print_r($order_goods_securitycode_list);
		//已经发货的
		...
		foreach($order_goods as $o_g){
		    for($i=0;$i<($o_g['total']-count($shipped_order_goods_securitycode_list[$o_g['id']]));$i++){
		        $not_shipped_order_goods_securitycode_list[$o_g['id']][]=array('order_id'=>$o_g['orderid'],'order_goods_id'=>$o_g['id'],'securitycode'=>'');
		    }
		}
		//组合已发货和未发货
		$all_order_goods_securitycode_list=array();
		foreach($shipped_order_goods_securitycode_list as $key=>$shipped_order_goods_securitycode_list2){
		    foreach($shipped_order_goods_securitycode_list2 as $shipped_order_goods_securitycode){
		        $all_order_goods_securitycode_list[$key][]=$shipped_order_goods_securitycode;
		    }
		}
		foreach($not_shipped_order_goods_securitycode_list as $key=>$not_shipped_order_goods_securitycode_list2){
		    foreach($not_shipped_order_goods_securitycode_list2 as $not_shipped_order_goods_securitycode){
		        $all_order_goods_securitycode_list[$key][]=$not_shipped_order_goods_securitycode;
		    }
		}
        //print_r($all_order_goods_securitycode_list);
	 ...
	 * 订单审核通过
	 */
	public function audit_not_passed(){
	    global $_W;
	    global $_GPC;
	    
	    $id=$_GPC['id'];
	    $checked=$_GPC['checked']?$_GPC['checked']:0;
	    pdo_update("ewei_shop_order", array( "checked" => $checked,), array( "id" => $id, "uniacid" => $_W["uniacid"] ));
	    $order_info = pdo_fetch("SELECT checked FROM " . tablename("ewei_shop_order") . " WHERE id = :id and uniacid=:uniacid", array( ":id" => $id, ":uniacid" => $_W["uniacid"] ));
	    if($checked==23 && $order_info['checked']!=1){
	    show_json(0,'请先客服审核!');
	    }
	    if($checked==33 && $order_info['checked']!=2){
	    show_json(0,'请先财务审核!');
	    }
	    if($checked==43 && $order_info['checked']!=3){
	    show_json(0,'请先仓库审核!');
	    }
	    show_json(1);
	...
	public function audit_success(){
	    ...
	    $checked=$_GPC['checked']?$_GPC['checked']:0;
	    $order_info = pdo_fetch("SELECT checked FROM " . tablename("ewei_shop_order") . " WHERE id = :id and uniacid=:uniacid", array( ":id" => $id, ":uniacid" => $_W["uniacid"] ));
	    if($checked==2 && $order_info['checked']!=1){
	    show_json(0,'请先客服审核通过!');
	    }
	    if($checked==3 && $order_info['checked']!=2){
	    show_json(0,'请先财务审核通过!');
	    }
	    if($checked==4 && $order_info['checked']!=3){
	    show_json(0,'请先仓库审核通过!');
	    }
	    pdo_update("ewei_shop_order", array( "checked" => $checked,), array( "id" => $id, "uniacid" => $_W["uniacid"] ));
	    show_json(1);
	}
