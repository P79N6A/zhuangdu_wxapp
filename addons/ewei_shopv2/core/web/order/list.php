...
class List_EweiShopV2Page extends WebPage
{
	protected function orderData($status, $st)
	{
		...
		//Yoper add
		if($_GPC['audit_success']!=''){//
		    $condition.=" AND (o.audit_success={$_GPC['audit_success']}) ";
		}
		if($_GPC['wechat_level']!=''){//
		    $condition.=" AND (o.wechat_level={$_GPC['wechat_level']}) ";
		}
		...
		if( $status !== "" )
		{
			if( $status == "-2" )
			{
				$statuscondition = " AND o.status=-2 and o.refundtime=0";
				$priceStatus = " AND status=-2 and refundtime=0";
			}
			elseif( $status == "-1" )
			{
		...
		if(p('yoxwechatbusiness')){//
		    $level_list=p('yoxwechatbusiness')->level_list($uniacid);
		}
	...
	public function status_2()
	{
		global $_W;
		global $_GPC;
		$orderData = $this->orderData(-2, "status_2");
	}
