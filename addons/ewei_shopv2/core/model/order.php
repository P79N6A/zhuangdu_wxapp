...
class Order_EweiShopV2Model 
{
	...
	public function payResult($params) 
	{
		...
		$order = pdo_fetch("select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus,iscycelbuy,packageid from " . tablename("ewei_shop_order") . " where  ordersn=:ordersn and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":ordersn" => $ordersn ));
		...
		if( $params["from"] == "return" ) 
		{
			...
			if( $order["istrade"] == 0 ) 
			{
				if( $order["status"] == 0 ) 
				{
					...
					if(p("yoxwechatbusiness") ){
					    $wechatbusiness_user_info=p("yoxwechatbusiness")->user_info($order['uniacid'], $order['openid']);
					    $change_data['wechat_level']=$wechatbusiness_user_info['level'];
					}
					logging_run(__METHOD__.':order:');
					logging_run($order);
					logging_run(__METHOD__.':change_data:');
					logging_run($change_data);
					pdo_update("ewei_shop_order", $change_data, array( "id" => $order["id"] ));
					...
					try{
					if(p("yoxwechatbusiness") ){
					    //微商模块，购买了套餐，检查是否对应等级套餐，是则对其做升级处理
					    p("yoxwechatbusiness")->after_payment_process($order);
					}
					}catch (\Exception $e){
					    logging_run(__METHOD__.':catch');
					    logging_run($e->getMessage());
					}
					if( $order["isparent"] == 1 ) 
					{
			...
			logging_run(__METHOD__.__LINE__.': TRUE');
			return true;
		}
		else 
		{
		    logging_run(__METHOD__.__LINE__.':BEGIN FALSE');
			return false;
		}
	}
