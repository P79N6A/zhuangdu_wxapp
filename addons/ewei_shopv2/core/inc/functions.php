			...
			if ($options['type'] == 'image') {
				...
				if ($item['optiontitle']) {
					...
					foreach ($item['optiontitle'] as $option) {
						...
						$html .= '<input type=\'hidden\' value=\'' . $option['packageprice'] . ',' . $option['commission1'] . ',' . $option['commission2'] . ',' . $option['commission3'].','. $option['num']. ($total . $maxbuy . $totalmaxbuy . "'\r\n                        name='" . $options['selectorid'] . 'packagegoodsoption') . $option['optionid'] . '\' >';
					}
				}
				else {
