...
$type = in_array($op, $dos) ? $op : '';



if(empty($type)) {

	message('支付方式错误,请联系主办方', '', 'error');
...
if(!empty($type)) {
	...
	if($type == 'wechat') {
		...
		if(!empty($plid)) {
			...
			pdo_update('core_paylog', array('openid' => $payopenid, 'tag' => iserializer($tag)), array('plid' => $plid));
		...
		$auth_cash = sha1($sl . $_W['uniacid'] . $_W['config']['setting']['authkey']);

		$params = $ps;

		require MODULE_ROOT . '/payment/wechat/pay.php';
