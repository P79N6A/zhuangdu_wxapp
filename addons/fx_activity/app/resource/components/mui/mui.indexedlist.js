...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		caleLayout: function() {
			...
			var barItemHeight = ((self.el.bar.offsetHeight - 100) / self.el.barItems.length) + 'px';
		...
		search: function(keyword) {
			...
			var checkGroup = function(currentIndex, last) {
				if (groupIndex >= 0 && itemCount >= currentIndex - groupIndex - (last ? 0 : 1)) {
					hiddenBuffer.push(classSelector('indexed-list-inner li') + ':nth-child(' + (groupIndex + 1) + ')');
			...
			liArray.forEach(function(item) {
				...
				if (item.classList.contains($.className('indexed-list-group'))) {
					...
					if (text.indexOf(keyword) >=0 || value.indexOf(keyword) >=0 || tags.indexOf(keyword) >=0) {
			...
			if (selectorBuffer.length == 0 && keyword) {
				self.el.inner.classList.add('empty');
			} else if (selectorBuffer.length > 0) {
				self.el.inner.classList.add('hidden');
				self.el.inner.classList.remove('empty');
				var innerText = selectorBuffer.join(', ') + "{display:inherit!important;}";
				if (hiddenBuffer.length > 0)
					innerText = innerText + hiddenBuffer.join(', ') + "{display:none;}";
				self.el.styleForSearch.innerText = innerText;
			} else {
				self.el.inner.classList.remove('empty');
				self.el.inner.classList.remove('hidden');
