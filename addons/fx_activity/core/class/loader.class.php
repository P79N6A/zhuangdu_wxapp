...
class fx_loader {
	...
	function web($name) {
		...
		$file = MODULE_ROOT . '/web/common/' . $name . '.func.php';
		if (file_exists($file)) {
			include_once $file;
			$this->cache['wnweb'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid Web Helper ' . MODULE_ROOT . '/web/common/' . $name . '.func.php', E_USER_ERROR);
			return false;
		}
	}
	...
	function app($name) {
		...
		$file = MODULE_ROOT . '/app/common/' . $name . '.func.php';
		if (file_exists($file)) {
			include_once $file;
			$this->cache['wnapp'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid App Function ' . MODULE_ROOT . '/app/common/' . $name . '.func.php', E_USER_ERROR);
			return false;
		}
	}
