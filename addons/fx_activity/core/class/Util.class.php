...
class Util 
{
	...
	*/
 	static function getCache($key,$name) {
		...
		return cache_read('fx_activity:'.$_W['uniacid'].':'.$key.':'.$name);
	}
	/** 
	* 设置缓存数据
	* 
	...
	*/
	static function setCache($key,$name,$value) {
		global $_W;
		if(empty($key) || empty($name)) return false;	
		$res = cache_write('fx_activity:'.$_W['uniacid'].':'.$key.':'.$name,$value);
		return $res;
	}
	...
	*/
	static function deleteCache($key,$name) {
		global $_W;		
		if(empty($key) || empty($name)) return false;
		return cache_delete('fx_activity:'.$_W['uniacid'].':'.$key.':'.$name);
	}
	...
	* @return array 
	*/  
	static function fx_log($filename,$path,$filedata){
	...
	static function getOneMemberByCache(){
		...
		if(empty($addresss)){
			$addresss = pdo_fetchall("SELECT openid,city FROM".tablename('fx_address')."WHERE 1 limit 1,10000");
			self::setCache('orderTip','10000',$addresss);
		}
		$index = rand(1, count($addresss));
		$address = $addresss[$index] ;
		$member = pdo_fetch("select nickname,avatar from ".tablename('fx_member')." where  openid='{$address['openid']}'");
