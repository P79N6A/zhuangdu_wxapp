(function(window) {
	...
	util.loading = function(msg, speed) {
		speed || (speed = 1500);
		var modalobj = $("#tips-container");
		0 == modalobj.length && ($(document.body).append('<div id="tips-container" class="text-center" style="top: 0;height: 100%;padding-top: 250px;"><span></span></div>'), modalobj = $("#tips-container"));
		var reg = /^\".*\"$/;
		msg = reg.test(msg) ? eval(msg) : util.decode(msg), modalobj.hide().find("span").html(msg), $("#tips-container").fadeIn();
	};
	util.loaded=function(){var e=$("#tips-container");e.length>0&&(e.modal("hide"),e.hide())}
	util.maps = function(val, callback, tpl){
			...
			modalobj.find('button.btn-primary').on('click', function(){
				if($.isFunction(callback)) {
					...
					var adinfo = modalobj.find("iframe").contents().find("#ad_info").val();
					var address = modalobj.find("iframe").contents().find("#addr_cur").val();
					var val = {lng: point.lng, lat: point.lat, adinfo : adinfo, label: address};
					callback(val);
				}
				modalobj.modal('hide');
			});
