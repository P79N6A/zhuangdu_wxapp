...
function file_upload($file, $type = 'image', $name = '', $compress = false) {
	$harmtype = array('asp', 'php', 'jsp', 'js', 'css', 'php3', 'php4', 'php5', 'ashx', 'aspx', 'exe', 'cgi','dll','java','c','py');
	if (empty($file)) {
	...
	switch ($type) {
		...
		case 'voice':
		case 'audio':
			$allowExt = array('mp3', 'wma', 'wav', 'amr','silk');//silk是微信语音文件-Yoper add//
		...
		//文档//Yoper add//
		case 'document':
		    $allowExt = array('txt', 'docx', 'doc','doc','xlsx','xls');
		    $limit=5;
	}
